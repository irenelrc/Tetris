...
Xwindow::Xwindow(int width, int height): width(width), height(height) {
  ...
  XColor xcolour;
  Colormap cmap;
  char color_vals[11][15]={"black", "papayawhip", "brown", "tomato", "darkorange", "lightsalmon", "yellowgreen", "powderblue", "cadetblue", "darkslategray", "grey19"};
...
void Xwindow::drawString(int x, int y, string msg, int colour) {
    XSetForeground(d, gc, colours[colour]);
    Font f = XLoadFont(d, "-apple-chalkduster-medium-r-normal--0-0-0-0-p-0-iso10646-1");
    XTextItem ti;
...
void Xwindow::drawBigString(int x, int y, string msg, int colour) {
    XSetForeground(d, gc, colours[colour]);
    // Font f = XLoadFont(d, "-linotype-avenir book-medium-r-normal--0-0-0-0-p-0-adobe-standard");
    ostringstream name;
    name << "-linotype-avenir book-medium-r-normal--0-0-" << 70 << "-" << 70 << "-p-0-adobe-standard";
